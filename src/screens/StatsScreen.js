...
const STATUS_ORDER = ['new', 'learning', 'consolidating', 'performance-ready', 'shelved', 'ambition'];

const STATUS_BAR_COLOURS = {
  new:                 '#F77F00',
  learning:            '#D53E0F',
  consolidating:       '#FCBF49',
  'performance-ready': '#2E8A72',
  shelved:             '#4A7A8A',
  ambition:            '#7F79DD',
};

function StatusTransitionsChart({ compositions }) {
  const [width, setWidth] = useState(0);

  // Only include pieces with at least one status history entry
  const pieces = compositions
    .filter(c => c.statusHistory && c.statusHistory.length > 0)
    .sort((a, b) => {
      const aStart = a.statusHistory[0]?.date || a.createdAt || '';
      const bStart = b.statusHistory[0]?.date || b.createdAt || '';
      return aStart.localeCompare(bStart);
    });

  if (pieces.length === 0) return (
    <Text style={{ fontFamily: 'CormorantGaramond-Italic', fontSize: 14, color: COLOURS.textDim, fontStyle: 'italic' }}>
      Status history will appear here as you change pieces between statuses.
    </Text>
  );
  ...
  const allDates = pieces.flatMap(c => (c.statusHistory || []).map(e => e.date));
  const minDate = allDates.reduce((a, b) => a < b ? a : b, today);
  const maxDate = today;
  const totalDays = Math.max(1, (new Date(maxDate + 'T12:00:00') - new Date(minDate + 'T12:00:00')) / 86400000);

  const ROW_H = 28;
  const LABEL_W = 110;
  const padR = 8;
  const padT = 24; // space for month labels
  const barH = 14;
  const barY = (ROW_H - barH) / 2;

  const chartW = width - LABEL_W - padR;

  function dateToX(dateStr) {
    const days = (new Date(dateStr + 'T12:00:00') - new Date(minDate + 'T12:00:00')) / 86400000;
    return LABEL_W + (days / totalDays) * chartW;
  }

  // Build segments for each piece
  function buildSegments(comp) {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      if (entry.to) segments.push({ status: entry.to, start, end });
    }
    return segments;
  }
  ...
  const totalH = padT + pieces.length * ROW_H;
    ...
    <View onLayout={e => setWidth(e.nativeEvent.layout.width)}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
        {STATUS_ORDER.map(s => (
          <View key={s} style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
            <View style={{ width: 10, height: 10, borderRadius: 2, backgroundColor: STATUS_BAR_COLOURS[s] }} />
      ...
      {width > 0 && (
        <Svg width={width} height={totalH} viewBox={`0 0 ${width} ${totalH}`}>
          ...
          {monthTicks.map((t, i) => (
            <G key={i}>
              <Line x1={t.x} y1={0} x2={t.x} y2={totalH} stroke={COLOURS.glassBorderSubtle} strokeWidth="1" />
              <SvgText x={t.x + 3} y={14} fontSize="8" fill={COLOURS.textDim} fontFamily="Lato">{t.label}</SvgText>
            </G>
          ))}

          {/* Piece rows */}
          {pieces.map((comp, ri) => {
            const y = padT + ri * ROW_H;
            const segments = buildSegments(comp);
            const shortTitle = comp.title.length > 16 ? comp.title.slice(0, 15) + '…' : comp.title;
            return (
              <G key={comp.id}>
                {/* Row bg */}
                <Path
                  d={`M0,${y} H${width} V${y + ROW_H} H0 Z`}
                  fill={ri % 2 === 0 ? 'rgba(255,255,255,0.18)' : 'transparent'}
                />
                {/* Piece label */}
                <SvgText
                  x={LABEL_W - 6}
                ...
                {segments.map((seg, si) => {
                  ...
                  const x2 = Math.min(dateToX(seg.end), width - padR);
                  const segW = Math.max(2, x2 - x1);
                  return (
                    <G key={si}>
                      <Path
                        d={`M${x1},${y + barY} h${segW} v${barH} h${-segW} Z`}
                        fill={STATUS_BAR_COLOURS[seg.status] || COLOURS.textDim}
                        opacity={0.88}
                      />
                    </G>
                  );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingLeft: isDesktop ? 226 : 16, paddingBottom: 40 }}>
        ...
        <SectionTitle style={{ marginTop: 16 }}>Status transitions</SectionTitle>
        <GlassCard>
          <StatusTransitionsChart compositions={compositions} />
        </GlassCard>
