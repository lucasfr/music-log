...
export function LessonDetailModal({ visible, lesson, compositions, onClose, onDelete, onEdit }) {
  if (!lesson) return null;

  const compName = id => (compositions.find(c => c.id === id) || {}).title || null;
  // support both old (pieces) and new (segments renamed via lessonMode) storage keys
