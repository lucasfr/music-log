      ...
      <View style={{ flex: 1, backgroundColor: COLOURS.bg }}>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 48 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
            ...
            <View style={{ paddingHorizontal: 14, paddingVertical: 8, borderRadius: RADIUS.pill, backgroundColor: 'rgba(255,255,255,0.55)', shadowColor: COLOURS.glassShadow, shadowOffset:{width:0,height:2}, shadowOpacity:1, shadowRadius:6, elevation:2 }}>
              <Text style={{ fontFamily: 'Lato-Bold', fontSize: 13, color: COLOURS.text }}>
                ⚡ {session.energy > 0 ? `+${session.energy}` : session.energy} · {energyLabel}
              </Text>
            </View>
            {session.enjoyment ? (
              <View style={{ paddingHorizontal: 14, paddingVertical: 8, borderRadius: RADIUS.pill, backgroundColor: 'rgba(255,255,255,0.55)', shadowColor: COLOURS.glassShadow, shadowOffset:{width:0,height:2}, shadowOpacity:1, shadowRadius:6, elevation:2 }}>
                <Text style={{ fontFamily: 'Lato-Bold', fontSize: 13, color: COLOURS.text }}>❤️ {session.enjoyment}/5</Text>
              </View>
            ) : null}
          </View>
            ...
            <View style={{ marginBottom: 20 }}>
              ...
              {techSegs.map(seg => (
                <View key={seg.id} style={{ paddingLeft: 12, borderLeftWidth: 2, borderLeftColor: COLOURS.steel, marginBottom: 12 }}>
                  <Text style={{ fontFamily: 'Lato-Bold', fontSize: 14, color: COLOURS.text }}>{seg.group || seg.title || 'Technical work'}{seg.duration ? ` · ${seg.duration}m` : ''}</Text>
              ...
              {repSegs.map(seg => {
                ...
                return (
                  <View key={seg.id} style={{ paddingLeft: 12, borderLeftWidth: 2, borderLeftColor: COLOURS.navy, marginBottom: 12 }}>
                    <Text style={{ fontFamily: 'CormorantGaramond-Italic', fontSize: 15, color: COLOURS.text }}>📜 {name || 'Piece'}{seg.duration ? ` · ${seg.duration}m` : ''}</Text>
